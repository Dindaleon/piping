...
(function() {
  ...
  module.exports = function(ops) {
    ...
    if (cluster.isMaster) {
      ...
      watcher.on("addDir", function(file) {
        var id, ref, worker;
        console.log("[piping]".bold.red, "Directory", path.relative(process.cwd(), file), "has been added, reloading.");
        ref = cluster.workers;
        for (id in ref) {
          worker = ref[id];
          respawnPending = true;
      ...
      watcher.on("unlink", function(file) {
        var id, ref, worker;
        console.log("[piping]".bold.red, "File", path.relative(process.cwd(), file), "has been removed, reloading.");
        ref = cluster.workers;
        for (id in ref) {
          worker = ref[id];
          respawnPending = true;
      ...
      watcher.on("unlinkDir", function(file) {
        var id, ref, worker;
        console.log("[piping]".bold.red, "Directory", path.relative(process.cwd(), file), "has been removed, reloading.");
        ref = cluster.workers;
        for (id in ref) {
          worker = ref[id];
          respawnPending = true;
